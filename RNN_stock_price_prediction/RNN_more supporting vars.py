...
train_set=train.iloc[:,[1,4]].values
...
for i in range(60,len(train_set_scale)):
    X_train.append(train_set_scale[i-60:i,[0,1]])
    y_train.append(train_set_scale[i,1])
...
real_stock_price=test.iloc[:,[1,4]].values

#prediction requires past 60days of data. so we need to concatenate 
#train and test sets 
data_total=pd.concat((train[['Open','Close']],test[['Open','Close']]),axis=0)
#close price has comma in the value so replaced them and converted them to float
...
predicted_stock=np.append(X_test[:,0,0],predicted_stock)
predicted_stock=predicted_stock.reshape(-2,2)
predicted_stock=sc.inverse_transform(predicted_stock)

#visualizing the results
plt.plot(real_stock_price[1],color='red',label='real price')
plt.plot(predicted_stock[0],color='green',label='predicted price')
